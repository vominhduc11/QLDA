...
export default function Accessory() {
    ...
    function handelClickHeart(e, ele, Id) {
        ...
        if (sessionStorage.getItem("User")) {
            ...
            if (ele.classList.contains('bg-danger')) {
                axios.delete(`http://localhost:8080/api/deleteProductHeart/${Id}`)
                    .then(res => {
                        if (res.data === "success") {
    ...
    // gọi api
    useEffect(() => {
        axios.get("http://localhost:8080/api/getAllAccessory")
        ...
        <div>
            <div className="bg-white py-3">
                <Container style={{ width: 1200 }} className="d-flex align-items-center">
                    ...
                    <span className="mt-1" style={{ color: '#f72c0f' }}>Xả kho giá tốt</span>
                </Container>
            </div>
            <div className="pt-4">
                <Container style={{ width: 1200 }}>
                    {product.length === 0 || <div className="d-flex flex-wrap">
                        {product.slice(currentPage * 15).slice(0, 15).map(item =>
